...
const hardPattern = /(דיור|משכנתא|ארנונה|שכירות|חשמל|מים|גז|ביטוח|גן|מעון|חינוך|בית ספר|קייטנה|תינוק|הלווא|חוב|אשראי|מיסים)/i;

function addToMap(map: Map<string, number>, key: string, amount: number) { map.set(key, (map.get(key) || 0) + amount); }
  ...
  try {
    const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(key)}`, {
      ...
      body: JSON.stringify({ contents: [{ parts: [{ text: [
        ...
        "CreditCardTransaction הוא פירוט בלבד של צריכה שכבר עשויה להופיע ב-Transaction. אסור לחבר את הסכומים משני המקורות. מותר להשתמש בפירוט האשראי רק כדי להבין בתי עסק, תשלומים, תדירות, קטגוריות ומגמות.",
...
export async function POST() {
  try {
    ...
    const historyStart = new Date(start); historyStart.setUTCMonth(historyStart.getUTCMonth() - 6);
    const [plan, incomes, funds, budgets, loans, transactions, creditCardTransactions] = await Promise.all([
      ...
      prisma.creditCardTransaction.findMany({ where: { userId: user.id, purchaseDate: { gte: historyStart, lt: end }, type: { in: ["CHARGE", "REFUND"] } }, select: { amount: true, type: true, purchaseDate: true, merchant: true, installmentTotal: true, installmentNumber: true, category: { select: { name: true } } }, orderBy: { purchaseDate: "desc" }, take: 10000 }),
    ]);

    const currentTransactions = transactions.filter(t => t.transactionDate >= start && t.transactionDate < end).map(t => ({ type: t.type, kind: t.kind, amount: toNumber(t.amount), transactionDate: t.transactionDate, categoryId: t.categoryId, categoryName: t.category?.name ?? null })) satisfies FinancialTransaction[];
    ...
    const income = calculateOperatingIncome(currentTransactions);
    const configuredIncome = sum(incomes.map(x => x.monthlyAmount));
    const projectedIncome = income > 0 ? income : configuredIncome;
    const actualExpense = Math.max(0, calculateNetExpense(currentTransactions));
    const debtPrincipal = sum(currentTransactions.filter(t => t.kind === "LOAN_PRINCIPAL").map(t => Math.abs(t.amount)));
    const debtPayment = calculateDebtPayments(currentTransactions);
    const sinking = sum(funds.map(x => x.monthlyContribution));
    const savings = toNumber(plan.monthlySavingsTarget);
    const available = projectedIncome - actualExpense - debtPrincipal - sinking - savings;
    ...
    for (const t of allTransactions) {
      ...
      addToMap(categoryTotals, t.categoryName || "אחר", amount);
    ...
    for (const t of creditCardTransactions) {
      ...
      if (merchant) {
        const current = merchantStats.get(merchant) || { total: 0, count: 0, months: new Set<string>() };
        current.total += amount; current.count += 1; current.months.add(t.purchaseDate.toISOString().slice(0, 7)); merchantStats.set(merchant, current);
      }
      if ((t.installmentTotal || 0) > 1) { installmentRows += 1; installmentAmount += Math.abs(Number(t.amount)); }
    }

    const recurringMerchants = [...merchantStats.entries()].filter(([, s]) => s.count >= 3 && s.months.size >= 2 && s.total > 0).sort((a,b) => b[1].total-a[1].total).slice(0,10).map(([name,s]) => ({ name, purchases:s.count, months:s.months.size, sixMonthTotal:money(s.total), averagePurchase:money(s.total/s.count) }));
    const payload = {
      householdSize: Math.max(1, user.householdSize),
      currentMonth: month,
      income: money(income), configuredIncome: money(configuredIncome),
      actualExpenses: money(actualExpense), debtPrincipal: money(debtPrincipal), debtPayment: money(debtPayment),
      availableCashAfterCurrentCommitments: money(available),
      plannedSavings: money(savings), sinkingMonthly: money(sinking),
      emergencyFund: money(Number(plan.emergencyFundAmount)), emergencyTargetMonths: plan.emergencyTargetMonths,
    ...
    if (recurringMerchants.length) warnings.push(`נמצאו ${recurringMerchants.length} בתי עסק עם רכישות חוזרות בפירוט האשראי.`);
    if (installmentRows) warnings.push(`נמצאו ${installmentRows} רכישות בתשלומים בפירוט האשראי.`);
    return NextResponse.json({
      source:"rules",
      situation: available >= 0 ? `אחרי הוצאות בפועל, החזרי קרן, חיסכון וקופות נשארים ${money(available)} ₪.` : `הוצאות והתחייבויות החודש גבוהות מהיכולת ב-${money(Math.abs(available))} ₪.`,
      ...
      allocations: available > 0 ? [{ name:"מרווח הוצאות", amount:money(available), reason:"זה הסכום שנותר לפי מקור האמת הכספי לאחר ההוצאות והתחייבויות." }] : [],
      warnings,
      confidence: income > 0 && actualExpense > 0 ? "high" : "medium",
    });
  } catch {
    return NextResponse.json({ error: "לא ניתן לנתח את התוכנית כרגע" }, { status: 400 });
  }
}
