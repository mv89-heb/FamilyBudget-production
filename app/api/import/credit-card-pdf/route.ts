...
function monthKeys(text: string) { const keys = new Set<string>(); const re = /\b(202\d)[./-](0?[1-9]|1[0-2])[./-](0?[1-9]|[12]\d|3[01])\b/g; let m: RegExpExecArray | null; while ((m = re.exec(text))) keys.add(`${m[1]}-${String(Number(m[2])).padStart(2, "0")}`); return [...keys].sort(); }
function rowsByMonth(rows: PdfRow[]) { const out = new Set<string>(); for (const row of rows) out.add(row.date.slice(0, 7)); return out; }
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const currentMonths = rowsByMonth([...unique.values()]); const missingMonths = expectedMonths.filter(month => !currentMonths.has(month));
      if (missingMonths.length) {
        for (const month of missingMonths) {
          const targeted = chunks.filter(chunk => chunk.includes(month.slice(0, 4)) || new RegExp(`(?:^|\\D)${month.slice(5)}[./-]`, "m").test(chunk));
          for (const chunk of targeted) { const rows = await requestGemini(`IMPORTANT: Recover transactions for missing month ${month}. Extract every visible transaction whose original purchase date is in ${month}.\n${chunk}`); for (const row of rows) if (row.date.startsWith(month)) unique.set(fingerprint(row), row); }
        }
      }
      const rows = [...unique.values()]; await prisma.importJob.update({ where: { id: job.id }, data: { status: "COMPLETED", rowsDetected: rows.length, rowsAnalyzed: allRows.length, rowsImported: 0, rowsUpdated: 0, rowsSkipped: 0, completedAt: new Date() } });
      return NextResponse.json({ success: true, rowsDetected: rows.length, rowsAnalyzed: allRows.length, monthsDetected: [...rowsByMonth(rows)].sort(), missingMonths: expectedMonths.filter(month => !rowsByMonth(rows).has(month)) });
