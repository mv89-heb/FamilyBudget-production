...
function fingerprint(row: CardRow) {
...
export async function POST(req: Request) {
  try {
    ...
    const sanitized = sanitizeImportText(body.text);
    if (!sanitized) return NextResponse.json({ error: "לא נמצא טקסט קריא לאחר ניקוי" }, { status: 422 });
    ...
    try {
      ...
      return NextResponse.json({ success: true, source: "CREDIT_CARD_OCR", rowsImported: createdRows, rowsUpdated: updatedRows, rowsSkipped: Math.max(0, rows.length - uniqueRows.length), categoriesCreated: createdCategories, paymentMethodsCreated: createdPaymentMethods, privacy: "ocr-in-browser-sanitized-before-gemini" });
