...
export async function POST(req: Request) {
  try {
    const user = await requireUser(); const body = await req.json().catch(() => null) as { text?: unknown; reprocess?: unknown } | null; const reprocess = body?.reprocess === true;
    ...
    try {
      ...
      await prisma.$transaction(async tx => { for (const row of uniqueRows) {
        ...
        const matchedMethod = matchPaymentMethod(row.paymentMethodName, methods); const data = { type: row.type, kind: row.kind, amount: row.amount, purchaseDate: new Date(`${row.date}T00:00:00.000Z`), postingDate: row.postingDate ? new Date(`${row.postingDate}T00:00:00.000Z`) : null, merchant: row.merchant.trim(), note: row.note?.trim() || null, categoryId: category.id, paymentMethodId: matchedMethod?.id || null, installmentTotal: row.installmentTotal ?? null, installmentNumber: row.installmentNumber ?? null, fingerprint: fingerprint(row) };
        const id = existingByFingerprint.get(data.fingerprint); if (id) { await tx.creditCardTransaction.update({ where: { id }, data }); updatedRows++; continue; }
    ...
    if (error instanceof Error && error.message === "UNAUTHORIZED") return NextResponse.json({ error: "לא מורשה" }, { status: 401 }); console.error("Credit card OCR import failed", error); return NextResponse.json({ error: "לא ניתן לייבא את OCR האשראי" }, { status: 400 });
  }
}
