...
import { creditCardIdentityMatches } from "@/lib/import/credit-card-identity";
...
async function analyzeWithGemini(rawRows: unknown[][]) { if (!rawRows.length) return [] as GeminiRow[]; const headers = rawRows[0], dataRows = rawRows.slice(1), results: GeminiRow[] = []; for (let index = 0; index < dataRows.length; index += GEMINI_CHUNK_ROWS) { const chunk = [headers, ...dataRows.slice(index, index + GEMINI_CHUNK_ROWS)]; let lastError: unknown; for (const model of GEMINI_MODELS) { for (let attempt = 0; attempt <= GEMINI_RETRIES; attempt++) { try { results.push(...await requestGemini(model, chunk)); lastError = undefined; break; } catch (error) { lastError = error; if (error instanceof Error && ["GEMINI_NOT_CONFIGURED", "GEMINI_AUTH_FAILED", "GEMINI_EMPTY_RESPONSE", "GEMINI_INVALID_RESPONSE"].includes(error.message)) throw error; if (attempt < GEMINI_RETRIES) await new Promise((resolve) => setTimeout(resolve, 750 * (attempt + 1))); } } if (!lastError) break; } if (lastError) throw lastError; } return results.slice(0, MAX_ROWS); }
    ...
    if (source === "CREDIT_CARD") {
      ...
      await prisma.$transaction(async (tx) => {
        for (const row of uniqueRows) {
          ...
          const identityMatches = candidates.filter((candidate) => creditCardIdentityMatches({ date: row.date, type, amount: row.amount, merchant, note: row.note?.trim() || null, installmentNumber: null, installmentTotal: null, paymentMethodId }, candidate)); const identityMatch = identityMatches.length === 1 ? identityMatches[0] : null;
          const existingId = existingByFingerprint.get(fp) ?? identityMatch?.id; if (existingId) { await tx.creditCardTransaction.update({ where: { id: existingId }, data }); updatedRows++; } else { await tx.creditCardTransaction.create({ data: { ...data, userId: user.id } }); createdRows++; }
        }
      });
