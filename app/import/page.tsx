...
async function readApiResponse(response: Response) {
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    if (response.status === 404) throw new Error("כתובת שירות הייבוא לא נמצאה בשרת. ייתכן שהגרסה החדשה עדיין לא נפרסה.");
  ...
  try {
    ...
    throw new Error(`השרת החזיר JSON לא תקין (HTTP ${response.status}).`);
  }
}
