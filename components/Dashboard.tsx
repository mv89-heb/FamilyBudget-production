...
import { ArrowLeft, ArrowDownLeft, BarChart3, Bell, CheckCircle2, Layers3, PiggyBank, Wallet } from "lucide-react";
...
type PlanData = {
  summary: { availableVariable: number; emergencyProgress: number; essentialMonthly: number; emergencyMax: number; uncategorizedActual: number; unbudgetedActual: number; freeAfterLeisure: number; hasLeisureTarget: boolean };
...
export default function Dashboard() {
  ...
  if (error) return <div role="alert" className="rounded-2xl border border-red-100 bg-red-50 p-5 text-sm font-medium text-red-700" dir="rtl">{error}</div>;
  ...
  const uncategorized = p?.summary.uncategorizedActual || 0;
  const unbudgeted = p?.summary.unbudgetedActual || 0;
  const alertCount = (uncategorized > 0 ? 1 : 0) + (unbudgeted > 0 ? 1 : 0) + (p && p.summary.availableVariable < 0 ? 1 : 0);
  const periodLabel = new Intl.DateTimeFormat("he-IL", { month: "long", year: "numeric" }).format(new Date(`${month}-01T12:00:00`));

  return <div className="space-y-6" dir="rtl">
    <header className="page-header">
      <div><div className="eyebrow"><BarChart3 size={14} /> מרכז בקרה</div><h1 className="page-title">המצב המשפחתי, במבט אחד</h1><p className="page-subtitle">מה נכנס, מה יצא, מה נשאר — ומה דורש טיפול.</p></div>
      <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-2 shadow-sm"><label htmlFor="dashboard-month" className="sr-only">חודש</label><input id="dashboard-month" type="month" value={month} onChange={e => setMonth(e.target.value)} className="rounded-xl border-0 bg-slate-50 px-3 py-2 text-sm font-medium outline-none" /></div>
    </header>

    <section className="grid gap-4 md:grid-cols-3">
      <MetricCard title="יתרה פנויה בפועל" value={money(d.balance)} icon={<Wallet size={20} />} tone={d.balance >= 0 ? "primary" : "warn"} hint="הכנסות פחות הוצאות שוטפות והחזרי קרן; פירוט אשראי לא נספר שוב." />
      <MetricCard title="הכנסות מול הוצאות שוטפות" value={`${money(d.income)} / ${money(d.expense)}`} icon={<ArrowDownLeft size={20} />} tone={d.income >= d.expense ? "good" : "warn"} hint={`${periodLabel} · יתרה אחרי מימון: ${money(d.balance)}`} />
    ...
    <section className="card-elevated overflow-hidden">
      ...
      <div className="grid border-t border-slate-100 md:grid-cols-3">
        {uncategorized > 0 ? <Link href="/transactions" className="flex items-center justify-between gap-4 p-5 transition hover:bg-slate-50"><div><strong className="block text-sm text-slate-900">יש הוצאות שלא קיבלו קטגוריה</strong><span className="mt-1 block text-xs text-slate-500">{money(uncategorized)} דורשים שיוך כדי להבין לאן הכסף יוצא.</span></div><ArrowLeft size={17} className="shrink-0 text-slate-400" /></Link> : <div className="flex items-center gap-3 p-5"><CheckCircle2 size={18} className="text-emerald-600" /><div><strong className="block text-sm text-slate-900">הקטגוריות מסודרות</strong><span className="mt-1 block text-xs text-slate-500">אין כרגע הוצאה שמסומנת כ"אחר" או "לא סווג".</span></div></div>}
        {unbudgeted > 0 ? <Link href="/plan" className="flex items-center justify-between gap-4 border-t border-slate-100 p-5 transition hover:bg-slate-50 md:border-t-0 md:border-r"><div><strong className="block text-sm text-slate-900">הוצאות ללא מסגרת</strong><span className="mt-1 block text-xs text-slate-500">{money(unbudgeted)} הן הוצאות אמיתיות, אך עדיין אין להן תקציב מוגדר.</span></div><ArrowLeft size={17} className="shrink-0 text-slate-400" /></Link> : <div className="flex items-center gap-3 border-t border-slate-100 p-5 md:border-t-0 md:border-r"><CheckCircle2 size={18} className="text-emerald-600" /><div><strong className="block text-sm text-slate-900">כל ההוצאות במסגרת</strong><span className="mt-1 block text-xs text-slate-500">לכל קטגוריה יש מסגרת חודשית.</span></div></div>}
        {p && p.summary.availableVariable < 0 ? <Link href="/plan" className="flex items-center justify-between gap-4 border-t border-slate-100 p-5 transition hover:bg-slate-50 md:border-t-0 md:border-r"><div><strong className="block text-sm text-slate-900">היתרה הפנויה שלילית</strong><span className="mt-1 block text-xs text-slate-500">נדרש איזון של ההוצאות, החיסכון או החזרי הקרן.</span></div><ArrowLeft size={17} className="shrink-0 text-slate-400" /></Link> : <Link href="/plan" className="flex items-center justify-between gap-4 border-t border-slate-100 p-5 transition hover:bg-slate-50 md:border-t-0 md:border-r"><div><strong className="block text-sm text-slate-900">הצעד הבא: תוכנית החודש</strong><span className="mt-1 block text-xs text-slate-500">בדיקה קצרה של תכנון מול ביצוע.</span></div><ArrowLeft size={17} className="shrink-0 text-slate-400" /></Link>}
      </div>
    </section>

    <div className="grid gap-5 lg:grid-cols-[1.05fr_.95fr]">
      <section className="card-elevated p-5">
        <div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-extrabold text-slate-900">איפה הכסף יוצא?</h2><p className="mt-1 text-xs text-slate-400">הקטגוריות הגדולות של {periodLabel} · ללא החזרי קרן</p></div><Link href="/transactions" className="text-sm font-bold text-indigo-600">לתנועות <ArrowLeft size={14} className="inline" /></Link></div>
        {d.byCategory.length === 0 ? <EmptyState text="אין הוצאות בתקופה הזו." /> : <div className="space-y-4">{d.byCategory.slice(0, 6).map(x => <div key={x.name}><div className="mb-1.5 flex justify-between gap-3 text-sm"><span className="font-medium text-slate-700">{x.name}</span><b className="text-slate-900">{money(x.amount)}</b></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-slate-700 transition-all" style={{ width: `${Math.min(100, Math.max(0, (x.amount / (d.expense || 1)) * 100))}%` }} /></div></div>)}</div>}
      </section>
      <section className="card-elevated p-5">
...
function QuickAction({ href, icon, title, text }: { href: string; icon: ReactNode; title: string; text: string }) { return <Link href={href} className="card-elevated flex items-center gap-3 p-4 transition hover:-translate-y-0.5 hover:shadow-md"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-slate-100 text-slate-600">{icon}</span><span><strong className="block text-sm text-slate-900">{title}</strong><span className="text-xs text-slate-500">{text}</span></span><ArrowLeft size={15} className="mr-auto text-slate-400" /></Link>; }
function EmptyState({ text }: { text: string }) { return <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">{text}</div>; }
function DashboardSkeleton() { return <div className="space-y-6" dir="rtl"><div className="h-20 animate-pulse rounded-2xl bg-slate-100" /><div className="grid gap-4 md:grid-cols-3">{[1,2,3].map(x => <div key={x} className="h-32 animate-pulse rounded-2xl bg-slate-100" />)}</div><div className="h-52 animate-pulse rounded-2xl bg-slate-100" /><div className="grid gap-5 lg:grid-cols-2"><div className="h-80 animate-pulse rounded-2xl bg-slate-100" /><div className="h-80 animate-pulse rounded-2xl bg-slate-100" /></div></div>; }
