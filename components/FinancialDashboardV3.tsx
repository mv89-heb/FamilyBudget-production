...
export default function FinancialDashboardV3() {
  ...
  const debtService = data.debts?.monthlyPayments ?? (data.debtPrincipal + data.debtInterest);
  ...
  return <div dir="rtl" className="space-y-6 pb-8">
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><FinancialKpi label="הכנסות" value={income.value} status={income.status} tone="positive" icon={<TrendingUp size={18} />} hint={ledgerHasData ? "כל הכסף שנכנס ל-Ledger" : "אין נתוני תנועות לחודש"} /><FinancialKpi label="הוצאות שוטפות" value={operatingExpense.value} status={operatingExpense.status} icon={<TrendingDown size={18} />} hint="צריכה, חשבונות, ריבית והוצאות תפעוליות" /><FinancialKpi label="תשלומי חוב" value={debtService} status={ledgerHasData ? "HAS_DATA" : "NO_DATA"} icon={<Wallet size={18} />} hint="סך התשלומים החודשיים שזוהו בהתחייבויות" /><FinancialKpi label="תזרים נטו" value={netFlow.value} status={netFlow.status} tone={data.netCashFlow >= 0 ? "positive" : "negative"} icon={<Wallet size={18} />} hint="הכנסות פחות כל היציאות בפועל" /></section>
    <section className="card-elevated p-5 md:p-6"><SectionHeader title="תזרים מזומנים — מה קרה בעו״ש?" description="כאן רואים את הכסף שיצא בפועל. החזר קרן אינו נעלם — אבל הוא עדיין יציאה מהחשבון." /><div className="mt-5 grid gap-3 sm:grid-cols-3"><FlowItem label="נכנס" value={data.income} note="הכנסות בפועל" tone="positive" /><FlowItem label="יצא בפועל" value={cashOutflow} note={`הוצאות שוטפות ${money(data.expense)} + קרן ${money(data.debtPrincipal)}`} /><FlowItem label="תזרים נטו" value={data.netCashFlow} note="זה לא יתרת עו״ש; זו תנועת החודש" tone={data.netCashFlow >= 0 ? "positive" : "negative"} /></div>{data.debtPrincipal > 0 && <div className="mt-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm text-indigo-900"><strong>חשוב:</strong> {money(data.debtPrincipal)} מתוך היציאות החודשיות הן החזרי קרן. הן מופיעות כאן כיציאה מלאה, ובמקביל מופיעות בנפרד בבניית ההון כהקטנת חוב.</div>}</section>
