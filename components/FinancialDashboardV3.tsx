...
import { ArrowLeft, AlertTriangle, CheckCircle2, ChevronDown, PiggyBank, TrendingDown, TrendingUp, Wallet } from "lucide-react";
...
function CategoryList({ categories, expense }: { categories: Category[]; expense: number }) {
  ...
  return <div className="mt-5 space-y-2">
    {visible.map((row, i) => {
      ...
      return <a key={`${row.categoryId}-${row.categoryName}`} href="/transactions" className={`group flex items-center gap-3 rounded-xl border px-3 py-3 transition hover:border-slate-300 hover:bg-slate-50 ${unknown ? "border-amber-200 bg-amber-50/60" : "border-transparent bg-white"}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 text-xs text-slate-500">{row.sharePercent}% מההוצאות · {unknown ? "לחצו כדי לפתוח את התנועות ולסווג" : "קטגוריה חשבונאית קיימת"}</div>
        </div>
    ...
    <div className="pt-1 text-xs text-slate-400">סה״כ הוצאות שוטפות: {money(expense)} · שום סכום לא מוסתר בתוך "אחר".</div>
  </div>;
}
...
export default function FinancialDashboardV3() {
  ...
  return <div dir="rtl" className="space-y-6 pb-8">
    <header className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
      <div><div className="eyebrow"><Wallet size={14} /> סקירה פיננסית</div><h1 className="page-title">מה קורה עם הכסף?</h1><p className="page-subtitle">{periodLabel} · הפרדה ברורה בין כסף שיצא מהחשבון לבין בניית הון.</p></div>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <FinancialKpi label="הכנסות" value={income.value} status={income.status} tone="positive" icon={<TrendingUp size={18} />} hint={ledgerHasData ? "כל הכסף שנכנס ל-Ledger" : "אין נתוני תנועות לחודש"} />
      <FinancialKpi label="הוצאות שוטפות" value={operatingExpense.value} status={operatingExpense.status} icon={<TrendingDown size={18} />} hint="צריכה, חשבונות, ריבית והוצאות תפעוליות" />
    ...
    <section className="text-center text-xs text-slate-400">{data.transactionCount > 0 ? `${number(data.transactionCount)} תנועות בחודש · הנתונים מבוססים על ה-Ledger` : "אין תנועות בחודש הזה · לא מוצגים אפסים כאילו הם נתונים אמיתיים"}</section>
  </div>;
}
...
function BudgetRow({ row }: { row: Budget }) { const tone = row.status === "OVER" ? "bg-red-500" : row.status === "WARNING" ? "bg-amber-400" : "bg-emerald-500"; return <div><div className="mb-1 flex items-center justify-between gap-3 text-sm"><span className="truncate font-bold text-slate-700">{row.categoryName}</span><span className="font-semibold text-slate-500">{money(row.spent)} / {money(row.limit)}</span></div><div className="h-2 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${tone}`} style={{ width: `${row.progressPercent}%` }} /></div></div>; }
