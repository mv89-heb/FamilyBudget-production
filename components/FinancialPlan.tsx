...
export default function FinancialPlan() {
  ...
  return <main className="page-shell decision-page" dir="rtl">
    <div className="page-header"><div><p className="eyebrow">קבלת החלטות · החודש הנוכחי</p><h1>תוכנית החודש</h1><p>שלושה מספרים כדי להבין איפה אנחנו, ואז פירוט רק כשצריך.</p></div><span className="data-status">{coverageLabel}</span></div>
    ...
    <section className="decision-callout"><div><strong>מה עושים עכשיו?</strong><p>{s.recommendation}</p></div><div className="decision-callout-side"><span>{s.hasLeisureTarget ? "פנוי אחרי יעד פנאי חודשי" : "יעד פנאי חודשי"}</span><strong>{s.hasLeisureTarget ? money(s.freeAfterLeisure) : "לא הוגדר"}</strong></div></section>

    <section className="decision-card"><div className="decision-card-header"><div><h2>התקדמות התקציב</h2><p>אחוז מוצג רק כשקיימת מסגרת תקציבית אמיתית.</p></div></div><div className="overview-progress-grid">
      <div><div className="progress-heading"><span>הוצאות קשיחות</span><strong>{s.hardBudgetLimit > 0 ? `${Math.round(hardPercent)}%` : "ללא מסגרת"}</strong></div>{s.hardBudgetLimit > 0 ? <><div className={`decision-progress ${progressTone(hardPercent)}`}><div style={{ width: `${hardPercent}%` }} /></div><small>{money(s.hardActual)} / {money(s.hardBudgetLimit)}</small></> : <small>{money(s.hardActual)} בפועל · לא הוגדרה מסגרת</small>}</div>
      <div><div className="progress-heading"><span>הוצאות משתנות</span><strong>{s.variableBudgetLimit > 0 ? `${Math.round(variablePercent)}%` : "ללא מסגרת"}</strong></div>{s.variableBudgetLimit > 0 ? <><div className={`decision-progress ${progressTone(variablePercent)}`}><div style={{ width: `${variablePercent}%` }} /></div><small>{money(s.variableActual)} / {money(s.variableBudgetLimit)}</small></> : <small>{money(s.variableActual)} בפועל · לא הוגדרה מסגרת</small>}</div>
    </div></section>

    <section className="decision-card"><div className="decision-card-header"><div><h2>לאן הכסף הולך?</h2><p>רק קטגוריות עם מסגרת מוצגות כאן. הוצאות ללא מסגרת מטופלות בנפרד.</p></div></div><div className="budget-accordion">
      {Object.entries(grouped).length === 0 && <div className="empty-state">עדיין לא הוגדרו תקציבים לחודש הזה.</div>}
      {Object.entries(grouped).map(([section, items]) => { const totalLimit = items.reduce((sum, item) => sum + item.limit, 0); const totalActual = items.reduce((sum, item) => sum + item.actual, 0); const percent = totalLimit > 0 ? Math.min(100, (totalActual / totalLimit) * 100) : 0; return <details className="budget-section" key={section}><summary><div><strong>{section}</strong><small>{items.length} סעיפים · {money(totalActual)} מתוך {money(totalLimit)}</small></div><div className="budget-summary"><b>{Math.round(percent)}%</b><span className={`mini-progress ${progressTone(percent)}`}><i style={{ width: `${percent}%` }} /></span></div></summary><div className="budget-section-body">{items.map(item => <div className="budget-item" key={item.categoryId}><div><strong>{item.name}</strong><small>{item.class === "HARD" ? "הוצאה קשיחה" : "הוצאה משתנה"} · {item.remaining >= 0 ? `נשארו ${money(item.remaining)}` : `חריגה של ${money(Math.abs(item.remaining))}`}</small></div><div className="budget-item-right"><b>{money(item.actual)}</b><span>{Math.round(item.percent)}%</span></div></div>)}</div></details>; })}
    </div>
    {s.uncategorizedActual > 0 && <><div className="attention-note"><strong>{money(s.uncategorizedActual)} ללא מסגרת</strong><span>זו הוצאה אמיתית, אבל אין לה כרגע תקציב מוגדר. כדאי לשייך או להגדיר מסגרת כדי לקבל תמונת תקציב אמינה.</span></div><div className="budget-section-body">{data.unbudgetedCategories.slice(0, 6).map(item => <div className="budget-item" key={item.categoryId}><div><strong>{item.name}</strong><small>ללא מסגרת תקציבית</small></div><div className="budget-item-right"><b>{money(item.actual)}</b></div></div>)}</div></>}
    </section>

    <details className="decision-card settings-accordion"><summary><div><h2>הגדרות ותכנון</h2><p>הכנסות, חיסכון, קרן חירום וקופות — לא חלק מהמעקב היומיומי.</p></div><span>פתיחה</span></summary><div className="settings-body">
