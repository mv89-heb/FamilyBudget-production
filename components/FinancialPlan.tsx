...
type Data = { plan: { monthlySavingsTarget: number; weeklyLeisureBudget: number; emergencyFundAmount: number; emergencyTargetMonths: number }; householdSize: number; incomes: Income[]; funds: Fund[]; summary: { netIncome: number; fixedCommitments: number; sinkingMonthly: number; savings: number; availableVariable: number; weeklyLeisure: number; monthlyLeisure: number; essentialMonthly: number; emergencyMin: number; emergencyMax: number; emergencyProgress: number; debtPayment: number; debtBurden: number; freeAfterLeisure: number } };
...
export default function FinancialPlan() {
  ...
  const emergencyPercent = s.emergencyMax > 0 ? Math.min(100, (s.emergencyProgress / s.emergencyMax) * 100) : 0;
  return <main className="page-shell" dir="rtl">
    <div className="page-header"><div><p className="eyebrow">תכנון משפחתי</p><h1>תוכנית החודש</h1><p>המערכת מסדרת את הכסף לפי סדר עדיפויות: קודם חובה, אחר כך חיסכון וקופות, ורק אז כסף פנוי.</p></div></div>
    {error && <div className="error-banner">{error}</div>}
    <section className="metrics-grid">
      <article className="metric-card"><span>הכנסה נטו</span><strong>{money(s.netIncome)}</strong><small>{data.householdSize} נפשות</small></article>
      <article className="metric-card"><span>התחייבויות קשיחות</span><strong>{money(s.fixedCommitments)}</strong><small>התקציבים שסומנו כחובה</small></article>
      <article className="metric-card"><span>נשאר למשתנה</span><strong>{money(s.availableVariable)}</strong><small>אחרי חיסכון וקופות</small></article>
      <article className="metric-card"><span>בילויים לשבוע</span><strong>{money(s.weeklyLeisure)}</strong><small>{money(s.freeAfterLeisure)} נשארים אחרי יעד הפנאי החודשי</small></article>
    </section>
    <section className="content-grid">
      ...
      <div className="card"><div className="card-header"><div><h2>חיסכון וקרן חירום</h2><p>קודם משלמים לעצמנו, ורק אחר כך מתכננים את הכסף הפנוי.</p></div></div>
        ...
        <div className="progress-block"><div className="progress-label"><span>קרן חירום</span><strong>{money(s.emergencyProgress)} / {money(s.essentialMonthly * Number(plan.emergencyTargetMonths))}</strong></div><div className="progress-track"><div className="progress-fill" style={{ width: `${emergencyPercent}%` }} /></div><small>יעד בסיסי: {money(s.emergencyMin)} · יעד מרבי: {money(s.emergencyMax)}</small></div>
      </div>
    </section>
    ...
    <section className="recommendation-card"><strong>המספר החשוב ביותר</strong><p>{s.availableVariable > 0 ? `אחרי התחייבויות, חיסכון וקופות נשארים ${money(s.availableVariable)} להוצאות המשתנות. לכן זה הסכום שממנו צריך לתכנן סופר, דלק, בילויים ושונות.` : "כרגע אין מספיק כסף פנוי אחרי התחייבויות, חיסכון וקופות. לפני שמגדילים הוצאות משתנות כדאי לאזן את התוכנית."}</p>{s.debtBurden > 0 && <small>החזרי הלוואות חודשיים: {money(s.debtPayment)} ({Math.round(s.debtBurden * 100)}% מההכנסה).</small>}</section>
  </main>;
}
