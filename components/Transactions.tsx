  ...
  return (
    <div className="space-y-6">
      <header className="page-header">
        <div>
          <div className="eyebrow">ניהול פיננסי</div>
          <h1 className="page-title">תנועות</h1>
          <p className="page-subtitle">הוסף, ערוך וסקור את ההכנסות וההוצאות שלך במקום אחד.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2 rounded-2xl border bg-white p-2 shadow-sm">
          ...
          <select aria-label="בחירת תקופה" value={month === "all" ? "all" : "month"} onChange={e => { if (e.target.value === "all") { setMonth("all"); setPage(1); } else { setMonth(currentMonth()); setPage(1); } }} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-medium outline-none focus:border-indigo-500">
            <option value="month">חודש נבחר</option><option value="all">כל התקופות</option>
          </select>
          {month !== "all" && <input aria-label="בחירת חודש" type="month" value={month} onChange={e => { setMonth(e.target.value); setPage(1); }} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-indigo-500" />}
        </div>
      </header>

      <section className="card-elevated overflow-hidden">
        <div className="border-b border-slate-100 bg-slate-50/70 px-5 py-4 md:px-6">
          <div className="flex items-center justify-between gap-3"><div><h2 className="font-bold text-slate-900">{editing ? "עריכת תנועה" : "תנועה חדשה"}</h2><p className="mt-1 text-xs text-slate-500">הזן את הפרטים כדי לשמור את התנועה בחשבון.</p></div>{editing && <button type="button" onClick={() => { setEditing(null); setForm(initial()); }} className="text-sm font-medium text-slate-500 hover:text-slate-900">ביטול עריכה</button>}</div>
        </div>
