...
export type FinancialSourceOfTruth = {
  month: string; transactionCount: number; ledger: ReturnType<typeof calculateLedgerSummary>; savings: ReturnType<typeof calculateSavingsMetrics>; emergency: ReturnType<typeof calculateEmergencyFund>; budgets: ReturnType<typeof calculateBudgetComparisons>; categories: ReturnType<typeof calculateCategoryBreakdown>; insights: ReturnType<typeof calculateSmartInsights>["insights"];
  debts: { count: number; outstanding: number; monthlyPayments: number; principalPaid: number; interestPaid: number }; netWorth: ReturnType<typeof calculateNetWorth>;
...
function toLedgerTransaction(row: { type: "INCOME" | "EXPENSE"; kind: LedgerTransaction["kind"]; amount: unknown; transactionDate: Date; categoryId?: string | null; category?: { name: string } | null; note?: string | null }): LedgerTransaction { return { type: row.type, kind: row.kind, amount: Number(row.amount), transactionDate: row.transactionDate, categoryId: row.categoryId ?? null, categoryName: row.category?.name, note: row.note ?? null }; }
...
export async function getFinancialSourceOfTruth(userId: string, requestedMonth?: string): Promise<FinancialSourceOfTruth> {
  const month = requestedMonth || getIsraelMonth(); const range = monthRange(month); const historyStart = monthRange(monthBefore(month, 12)).start;
  ...
  const summary = calculateLedgerSummary(currentRows); const categories = calculateCategoryBreakdown(currentRows); const savings = calculateSavingsMetrics(summary, currentRows);
  const budgetsForEngine = budgets.map((budget) => ({ categoryId: budget.categoryId, categoryName: budget.category.name, limit: Number(budget.limit) })); const budgetComparisons = calculateBudgetComparisons(currentRows, budgetsForEngine);
  const emergencyMonths = Math.max(3, Math.min(6, plan?.emergencyTargetMonths ?? 3)); const hardBudgetTotal = budgets.filter((budget) => budget.class === "HARD").reduce((sum, budget) => sum + Number(budget.limit), 0); const emergencyTarget = hardBudgetTotal > 0 ? roundMoney(hardBudgetTotal * emergencyMonths) : roundMoney(summary.operatingExpense * emergencyMonths); const emergencyCurrent = emergencyFunds.reduce((sum, fund) => sum + Number(fund.currentAmount), 0) || Number(plan?.emergencyFundAmount ?? 0); const emergency = calculateEmergencyFund(emergencyCurrent, emergencyTarget);
  const historicalRows = allRows.filter((row) => row.transactionDate < range.start); const historicalCategories = calculateCategoryAmounts(historicalRows); const dominant = categories[0] ?? null; const historicalDominant = dominant ? historicalCategories.find((row) => row.categoryId === dominant.categoryId || row.categoryName === dominant.categoryName) : null;
