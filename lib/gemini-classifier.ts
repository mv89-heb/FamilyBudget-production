...
export class GeminiClassificationError extends Error {
  ...
  constructor(code: GeminiClassificationError["code"], message: string, status: number) { super(message); this.name = "GeminiClassificationError"; this.code = code; this.status = status; }
}
...
function extractJson(text: string) {
  ...
  try { return JSON.parse(cleaned.slice(start, end + 1)); } catch { throw new GeminiClassificationError("INVALID_RESPONSE", "Gemini החזיר תשובה שלא ניתן לאמת", 502); }
}

function normalize(value: string) { return value.toLocaleLowerCase("he").replace(/[\u200e\u200f\u202a-\u202e]/g, "").replace(/\s+/g, " ").trim(); }

function matchesRule(text: string, rule: ClassificationRule) {
  const value = normalize(text); const pattern = normalize(rule.pattern);
...
function findCategory(categories: ClassificationCategory[], name: string) { const target = normalize(name); return categories.find((category) => normalize(category.name) === target); }

async function loadRules(userId: string) {
  return prisma.classificationRule.findMany({
    where: { userId, active: true },
    select: { id: true, pattern: true, matchType: true, categoryId: true, priority: true, category: { select: { name: true } } },
...
export async function classifyTransactionsWithGemini(transactions: ClassificationTransaction[], categories: ClassificationCategory[], options: { userId: string }) {
  ...
  if (unresolved.length) {
    try { suggestions.push(...await classifyBatch(unresolved, categories)); }
    catch (error) { warning = error instanceof GeminiClassificationError ? error.message : "Gemini לא זמין כרגע"; }
  }
