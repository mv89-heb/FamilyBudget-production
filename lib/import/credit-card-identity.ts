...
type CreditCardDateRange = { gte: Date; lte: Date };

export function creditCardIdentityDateRange(date: string): CreditCardDateRange;
export function creditCardIdentityDateRange(rows: Array<{ date: Date }>): CreditCardDateRange | null;
export function creditCardIdentityDateRange(value: string | Array<{ date: Date }>) {
  ...
  return {
    ...
    lte: new Date(Math.max(...timestamps) + 86_399_999),
  };
}
